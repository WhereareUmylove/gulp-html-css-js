...
// 路径
var html_news_path = 'src/news/*.html'
var html_product_path = 'src/product/*.html'
var html_main_path = 'src/*.html'

var js_libs_path = 'src/static/js/product/*.js'
var js_main_path = 'src/static/js/*.js'
var css_libs_path = 'src/static/css/**/*.css'
var css_main_path = 'src/static/{css}/**/*.css'
var images_path = ['src/static/images/**', 'src/static/images/public/favicon.ico']
var fonts_path = 'src/static/**/fonts/**'
...
gulp.task('html_news', function () {
    return gulp.src(html_news_path)
        .pipe(htmltpl({
            // tag: 'component',
            // paths: ['src/common', 'src/components'],
            engine: function (template, data) {
                console.log(html_news_path)
                return template && artTemplate.compile(template)(data)
            },
            // 传入页面的初始化数据
            data: {
        ...
        .pipe(rename({
            dirname: '/' + html_news_path // 清空路径
        }))
        .pipe(gulp.dest('dist'))
})
gulp.task('html_product', function () {
    return gulp.src(html_product_path)
        .pipe(htmltpl({
            // tag: 'component',
            // paths: ['src/common', 'src/components'],
            engine: function (template, data) {
...
gulp.task('js_libs', function () {
    return gulp.src(js_libs_path)
        ...
        .pipe(concat('product.min.js')) // 合并文件并命名
        .pipe(gulpif(env === 'build', uglify())) // 判断是否压缩js
        .pipe(gulp.dest('dist/static/js/product'))
})
...
gulp.task('watch', function () {
    w(html_main_path, 'html')
    w(html_news_path, 'html_news')
    w(html_product_path, 'html_product')
...
gulp.task('dev', function (cb) {
    ...
    runSequence(
        ['clean'],
        ['html', 'html_product', 'html_news', 'js_libs', 'js_main', 'css_libs', 'css_main', 'images', 'fonts'],
        ['browser', 'watch'],
        cb)
})
...
gulp.task('build', function (cb) {
    ...
    runSequence(
        ...
        ['html', 'html_product', 'html_news', 'js_libs', 'js_main', 'css_libs', 'css_main', 'images', 'fonts'], // 不分先后的任务最好并行执行，提高效率
